function particles() {
   const espacoparticulas = document.getElementById("espacoparticulas"),
      ctx = document.getElementById("particulas").getContext("2d"), lineDistance = 200;
   ...
   function moveParticles() {
      for (let i = 0, n = arrayParticles.length; i < n; i++) {
         ...
         particle.pos.x = particle.pos.x - particle.radius >= window.innerWidth ? -(particle.radius) :
            particle.pos.x + particle.radius <= 0 ? window.innerWidth + (particle.radius) : particle.pos.x;
         particle.pos.y = particle.pos.y - particle.radius >= window.innerHeight ? -(particle.radius) :
            particle.pos.y + particle.radius <= 0 ? window.innerHeight + (particle.radius) : particle.pos.y;
         for (let e = 0; e < n; e++) {
            if (e === i) { continue; }
            const particle2 = arrayParticles[e], distance = getDistanceCoordinates(particle.pos, particle2.pos);
            if (distance <= 200) {
               drawLine(particle.pos, particle2.pos, ((lineDistance - distance + 5) / lineDistance), false);
            }
         }
   ...
   function collision(particle1, particle2, dx, dy) {
      const deltaVelocityX = particle1.vel.x - particle2.vel.x;
      const deltaVelocityY = particle1.vel.y - particle2.vel.y;
      if (deltaVelocityX * dx + deltaVelocityY * dy >= 0) {
         const angle = -Math.atan2(dy, dx), m1 = particle1.radius, m2 = particle2.radius,
            u1 = rotate(particle1.vel.x, particle1.vel.x, angle),
            u2 = rotate(particle2.vel.y, particle2.vel.y, angle),
            v1 = { x: u1.x * (m1 - m2) / (m1 + m2) + u2.x * 2 * m2 / (m1 + m2), y: u1.y },
            v2 = { x: u2.x * (m1 - m2) / (m1 + m2) + u1.x * 2 * m2 / (m1 + m2), y: u2.y },
            vFinal1 = rotate(v1.x, v1.y, -angle),
            vFinal2 = rotate(v2.x, v2.y, -angle);
         particle1.vel.x = vFinal1.x - (vFinal1.x / 40);
         particle1.vel.y = vFinal1.y - (vFinal1.y / 40);
         particle2.vel.x = vFinal2.x - (vFinal2.x / 40);
         particle2.vel.y = vFinal2.y - (vFinal2.y / 40);
      }
...
function particulas() {
   ...
   function desenhaLinha(posX, posY, posX2, posY2, opacidade, voltaLinha) {
      ...
      if (voltaLinha === true) { contexto.lineTo(posX, posY); }
   ...
   function desfocarParticulas() {
      ...
      let tempoEsperar = valorAleatorio(0.5, 3);
      ...
      function focarParticulas() {
         ...
         tempoEsperar = valorAleatorio(5, 20);
         particulas.style.transition = "filter " + tempoTransicao + "s linear";
         setTimeout(() => {
